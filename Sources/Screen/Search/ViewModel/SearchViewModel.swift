            ...
            .sink { [self] query in
                Task {
                    do {
                        switch searchMode {
                        case .workspaces:
                            let result = try await searchManager.searchWorkspaces(for: sessionManager.userId, with: query)
                            await MainActor.run {
                                items = result.map { ListItem(id: $0.meta.id, title: $0.title, emoji: "🔥") }
                            }
                        case .documents:
                            let result = try await searchManager.searchDocuments(for: sessionManager.userId, with: query)
                            await MainActor.run {
                                items = result.map { ListItem(id: $0.meta.id, title: $0.title, emoji: "🔥") }
                            }
                        }
